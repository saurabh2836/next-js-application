"use client";
import { ReloadIcon } from "@radix-ui/react-icons";
import Link from "next/link";
import Image from "next/image";
import { useSearchParams } from "next/navigation";
import { useEffect, useState } from "react";

import { globalSearch } from "@/lib/actions/general.action";


const GlobalResult = () => {
    const searchParams = useSearchParams();
    const [result, setResult] = useState([]);
    const [isLoading, setLoading] = useState(true);

    const global = searchParams.get("global");
    const type = searchParams.get("type");

    useEffect(() => {
        const fetchResult = async () => {
            setLoading(true);
            setResult([]);

            try {
                const res = await globalSearch({
                    query: global as string || "",
                    type
                })
                console.log(res);
                setResult(res.data);
            } catch (error) {
                console.error("Error fetching global search results:", error);
            } finally {
                setLoading(false);
            }
        };

        if (global) {
            fetchResult();
        }
    }, [global, type]);


    const renderLink = (type: string, item: any) => {
        switch (type) {
            case "question":
                return `/question/${item._id}`;
            case "answer":
                return `/question/${item.questionId}/#${item._id}`;
            case "user":
                return `/profile/${item.clerkId}`;
            default:
                return "/";
        }
    }

    return (
        <div className="absolute  top-full z-10 mt-3 w-full rounded-xl bg-light-800 py-5 shadow-sm dark:bg-dark-400">
            <GlobalResult />
            <div className="my-5 h-[1px] bg-light-700/50 dark:bg-dark-500/50" />
            <div className="space-y-5">
                <p className="text-dark400_light900 paragraph-semibold px-5">
                    Top Match
                </p>
                {isLoading ? (
                    <div className="flex-center flex-col px-5">
                        <ReloadIcon className="animate-spin text-dark400_light900" />
                        <p className="paragraph-regular text-dark400_light900">
                            Browsing the whole database ...
                        </p>
                    </div>
                ) : (
                    <div className="flex flex-col gap-2">
                        {result.length > 0 ? (
                            result?.map((item: GlobalSearchedItem, index) => (
                                <Link
                                    key={item.type + item.id + index}
                                    href={renderLink(item.type, item.id)}
                                    className="flex items-center gap-3 px-5 py-3 hover:bg-light-700 dark:hover:bg-dark-500"
                                >
                                    <Image
                                        src="/icons/tag.svg"
                                        alt="tags"
                                        width={18}
                                        height={18}
                                        className="invert-colors mt-1 object-contain"
                                    />
                                    <div className="flex flex-col">
                                        <p className="body-medium text-dark200_light800 line-clamp-1">
                                            {item.title}
                                        </p>
                                        <p className="text-light400_light500 small-medium mt-1 font-bold capitalize">
                                            {item.type}
                                        </p>
                                    </div>
                                </Link>
                            ))
                        ) : (
                            <div className="flex-center flex-col px-5">
                                <p className="text-5xl">🫣</p>
                                <p className="text-dark200_light800 body-regular px-5 py-2.5">
                                    Oops, no results found
                                </p>
                            </div>
                        )}
                    </div>
                )}
            </div>
        </div>
    );
};

export default GlobalResult;